...
def mostrar_diferencia_carbohidratos(total_carbohidratos_consumidos,
  ...
  if diferencia_carbohidratos > 0:
    st.write(
        f"Has consumido {int(diferencia_carbohidratos)} gramos de carbohidratos en exceso hoy. Considera ajustar tu ingesta."
    )
  elif diferencia_carbohidratos < 0:
    st.write(
        f"Te faltan {int(abs(diferencia_carbohidratos))} gramos de carbohidratos para alcanzar tu ingesta diaria. ¡Asegúrate de consumir suficientes carbohidratos!"
    )
    st.write(
        "**Te recomendamos que consumas los siguientes alimentos con más carbohidratos**"
    )
    top_carbs = df.nlargest(5, 'Carbs')
    st.write(top_carbs)
  else:
    st.write(
        "Estás en línea con tu ingesta diaria de carbohidratos. ¡Bien hecho!")
...
def mostrar_diferencia_grasas(total_grasas_consumidas, grasas_diarias):
  ...
  if diferencia_grasas > 0:
    st.write(
        f"Has consumido {int(diferencia_grasas)} gramos de grasas en exceso hoy. Considera ajustar tu ingesta."
    )
  elif diferencia_grasas < 0:
    st.write(
        f"Te faltan {int(abs(diferencia_grasas))} gramos de grasas para alcanzar tu ingesta diaria. ¡Asegúrate de consumir suficientes grasas!"
    )
  else:
...
def mostrar_diferencia_proteinas(total_proteinas_consumidas,
                                 proteinas_diarias):
  diferencia_proteinas = proteinas_diarias - total_proteinas_consumidas

  if diferencia_proteinas > 0:
    st.write(
        f"Has consumido {int(diferencia_proteinas)} gramos de proteínas en exceso hoy. Considera ajustar tu ingesta."
    )
  elif diferencia_proteinas < 0:
    st.write(
        f"Te faltan {int(abs(diferencia_proteinas))} gramos de proteínas para alcanzar tu ingesta diaria. ¡Asegúrate de consumir suficientes proteínas!"
    )
    st.write(
        f"**Te recomendamos que consumas los siguientes alimentos con más proteínas**"
    )
...
def mostrar_diferencia_calorias(total_calorias_consumidas, calorias_diarias):
  """
...
nivel_actividad = st.selectbox("Selecciona tu nivel de actividad física:", [
    "Sedentario", "Ligera actividad", "Moderada actividad", "Alta actividad",
    "Muy alta actividad"
])
...
if boton_calcular:
  ...
  if st.session_state["authentication_status"]:
    ...
    calorias_diarias = calcular_calorias_diarias(sexo, peso, altura, edad,
                                                 nivel_actividad)
    ...
    columnas = [
        "Fecha", "Calorias", "Carbohidratos", "Grasa", "Fibra", "Proteinas"
    ]
    df = pd.DataFrame(datos, columns=columnas)

    # Calcular calorías semanales
    df["DiaSemana"] = pd.to_datetime(df["Fecha"]).dt.strftime(
        '%A')  # Obtener el nombre del día de la semana
    ...
    df_ultimos_7_dias = df[df['Fecha'] >= (datetime.now() -
                                           pd.DateOffset(days=7))]

    # Crear gráfico de barras de calorías diarias de los últimos 7 días
    plt.figure(figsize=(10, 6))
    plt.bar(df_ultimos_7_dias["Fecha"],
            df_ultimos_7_dias["Calorias"],
            color='blue',
            alpha=0.7)
    ...
    plt.figure(figsize=(10, 6))
    plt.bar(df_ultimos_7_dias["Fecha"],
            df_ultimos_7_dias["Carbohidratos"],
            color='green',
            alpha=0.7)
    ...
    mostrar_diferencia_carbohidratos(
        df_ultimos_7_dias["Carbohidratos"].sum(),
        calcular_carbohidratos_diarios(calorias_diarias,
                                       df_ultimos_7_dias["Proteinas"].sum(),
